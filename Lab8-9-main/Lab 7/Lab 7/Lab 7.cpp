...
int main()
{
	...
	ofstream file;
	file.open("C:\\Users\\Lyx\\Desktop\\Lab8-9-main\\Lab 7\\log.txt", ios::app);
	SYSTEMTIME st;
	...
	if (a > 8 || a < 1) {
		...
		file << "The program is closed because invalid value entered" << endl;
		return 0;
	}
	cout << "l= ";
	cin >> b;
	...
	file << "The program has completed point a" << endl;//готова буква а
	...
	while (1) {
		...
		file << "The entered value is not in the range from 1 to 4" << endl;
	}
	if (f == 1) {
	...
	file << "The program has completed point b" << endl;//готова буква б
	...
	while (1) {
		...
		file << "The entered value is not in the range from 1 to 3" << endl;
	}
	if (f == h) {
	...
	file << "The program has completed point c" << endl;
	int z = 0;
	...
	while (1) {
		...
		file << "The entered value is not in the range from 1 to 2" << endl;
	}
	if (z == 1) {
		a = (a - 1) * 5;
		b = (8 - b) * 5;
		...
		file << "The board is drawn" << endl;
	}
	else {
		file << "The board is not drawn" << endl;
	}
	GetLocalTime(&st);
	file << "The program is closed at" << st.wHour << ":" << st.wMinute << ":" << st.wSecond << endl;
	file.close();
	system("pause");
}
